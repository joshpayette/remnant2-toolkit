import {
  Textarea as HeadlessTextarea,
  type TextareaProps as HeadlessTextareaProps,
} from '@headlessui/react';
import { clsx } from 'clsx';
import { forwardRef } from 'react';

export const BaseTextarea = forwardRef<
  HTMLTextAreaElement,
  { resizable?: boolean; textareaClassName?: string } & HeadlessTextareaProps
>(function Textarea(
  { className, textareaClassName, resizable = true, ...props },
  ref,
) {
  return (
    <span
      className={clsx([
        className,

        // Basic layout
        'relative grid h-full w-full',

        // Background color + shadow applied to inset pseudo element, so shadow blends with border in light mode
        'before:bg-surface-solid before:absolute before:inset-px before:rounded-[calc(theme(borderRadius.lg)-1px)] before:shadow',

        // Background color is moved to control and shadow is removed in dark mode so hide `before` pseudo
        'before:hidden',

        // Focus ring
        'after:pointer-events-none after:absolute after:inset-0 after:rounded-lg after:ring-inset after:ring-transparent sm:after:focus-within:ring-2 sm:after:focus-within:ring-blue-500',

        // Disabled state
        'has-[[data-disabled]]:opacity-50 before:has-[[data-disabled]]:bg-zinc-950/5 before:has-[[data-disabled]]:shadow-none',
      ])}
      data-slot="control"
    >
      <HeadlessTextarea
        className={clsx([
          textareaClassName,

          // Basic layout
          'relative block h-full box-border w-full appearance-none rounded-lg px-[calc(theme(spacing[3.5])-1px)] py-[calc(theme(spacing[2.5])-1px)] sm:px-[calc(theme(spacing.3)-1px)] sm:py-[calc(theme(spacing[1.5])-1px)]',

          // Typography
          'text-surface-solid text-base/6 placeholder:text-zinc-500 sm:text-sm/6',

          // Border
          'border-surface-solid/10 data-[hover]:border-surface-solid/20 border',

          // Background color
          'bg-surface-solid/5',

          // Hide default focus styles
          'focus:outline-none',

          // Invalid state
          'data-[invalid]:border-accent3-600 data-[invalid]:data-[hover]:border-accent3-600',

          // Disabled state
          'disabled:border-surface-solid/15 disabled:bg-surface-solid/[2.5%] data-[hover]:disabled:border-surface-solid/15',

          // Resizable
          resizable ? 'resize-y' : 'resize-none',
        ])}
        ref={ref}
        {...props}
      />
    </span>
  );
});
