...
export function mainBuildQuery({
  ...
  //! Once Window functions are supported in Vitess, can use `COUNT(*) OVER() as totalCount,`
...
interface MainBuildQueryCountProps {
  includeBuildVariants: boolean;
  percentageOwned: PercentageOwned;
  searchText: string;
  userId: string | undefined;
...
export function mainBuildQueryCount({
  includeBuildVariants,
  percentageOwned,
  searchText,
  userId,
  whereConditions,
}: MainBuildQueryCountProps): Prisma.PrismaPromise<
...
`;
  return prisma.$queryRaw<Array<{ totalCount: number }>>(query);
}
