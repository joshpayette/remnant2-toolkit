...
export function getRandomBuild(): BuildState {
  let randomBuild: BuildState = JSON.parse(JSON.stringify(INITIAL_BUILD_STATE));
  ...
  getArrayOfLength(2).forEach((_, index) => {
    ...
    if (!randomSkill) {
      throw new Error(`Could not find random skill for ${randomArchtype.name}`);
    }
