...
export async function handleRelicFragmentItems() {
  ...
  for (const item of relicFragmentItems) {
    ...
    try {
      ...
      const newData = relicfragmentDataParse(cheerio.load(text), item);
      const dataComparison = relicfragmentDataCompare(newData, item);
