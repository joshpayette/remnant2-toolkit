...
export async function handleConcoctionItems() {
  const envVars = validateEnv()

  for (const item of concoctionItems) {
    ...
    try {
      ...
      const newData = concoctionDataParse(cheerio.load(text))
      const dataComparison = concoctionDataCompare(newData, item)
