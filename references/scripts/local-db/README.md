...
First, open a terminal and browse to `references/scripts/local-db`. Then, run the following command to dump the database from PlanetScale. This will create a folder with all the SQL files needed to recreate the database locally.
...
```bash
cat pscale/*.sql | mysql -h 127.0.0.1 -P 3306 remnant2toolkit -u root -p
```
