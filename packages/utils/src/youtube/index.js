 ...
 *  => 'watch'
 */
function getVideoUrlType(videoUrl) {
    if (!videoUrl) {
        return 'invalid';
    }
 ...
 */
function removeAllParamsExceptV(videoUrl) {
 ...
 *  => '4kIQKHxZGS8'
 */
function videoUrlToVideoId(videoUrl) {
 ...
 */
function videoEmbedUrlToWatchUrl(videoEmbedUrl) {
    ...
    return `https://www.youtube.com/watch?v=${videoId}${startValue ? `&t=${startValue}` : ''}`;
}
/**
 * Determines whether a provided URL is a valid Youtube video URL
 * @example
 ...
 * => false
 */
export function isValidYoutubeUrl(videoUrl) {
 ...
 */
export function videoUrlToThumbnailUrl(videoUrl) {
 ...
 *  => 'https://www.youtube.com/watch?v=4kIQKHxZGS8'
 */
export function videoUrlToWatchUrl(videoUrl) {
    const videoUrlType = getVideoUrlType(videoUrl);
